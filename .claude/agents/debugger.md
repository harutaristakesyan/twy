...
- **`@shared/index` symbol not resolved** → the path alias is internal to `@twy/functions` (`tsconfig.paths.json`). Rebuild that package: `pnpm --filter @twy/functions build`.
- **SST deploy fails with a Pulumi state lock** → another deploy is in flight or a prior one crashed. `pnpm sst unlock --stage <stage>` (CI does this automatically).
...
- **Mutation succeeds but the list doesn't refresh** → no `queryClient.invalidateQueries({ queryKey: […] })` in `onSuccess`, or the key doesn't match the list page's key.
- **Cookie-based JWT not present after redirect** → cookies are scoped per origin. Users on `twy.am` vs `twy.be` are independent sessions by design (root CLAUDE.md). Don't try to share — that's a feature.
...
- **Aurora Data API call returns `AccessDeniedException`** → the running role lacks `rds-data:ExecuteStatement` (or `secretsmanager:GetSecretValue` on the cluster's managed secret). Both should come automatically from `link: [db.cluster]` — verify the route in `infra/routes.ts` includes `"cluster"` in `linkKeys`, or that the migration runner is launched via `sst shell` (which inherits the deploy role's permissions).
- **Data API throws `BadRequestException: Transaction <id> is not in a state…`** → two `tx.X` calls were fired concurrently inside `db.transaction(...)`. The Data API serialises one statement per `transactionId`; use sequential `await`s, never `Promise.all([tx.X, tx.Y])`.
