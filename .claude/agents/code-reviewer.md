...
If the diff is large (>30 files), focus on Lambda handlers, migrations, `infra/` modules, and React hooks. Skim the rest.
...
- **`process.env.X!` non-null assertions for SST-managed values**. Read `Resource.X` (the SST SDK) instead. `requireEnv()` is deprecated.
...
- `noNonNullAssertion` is `error` — flag every `!`.
...
- UI components come from `@heroui/react` (HeroUI v3); icons from `lucide-react`. AntD is gone — flag any `import ... from "antd"` or `@ant-design/...`.
- Server state goes through TanStack Query v5 (`useQuery` / `useMutation`). `ahooks` is gone — flag `useAntdTable` / `useRequest`.
- Modals are route-based: opened via `navigate("create")`, closed via `navigate("..")`. Flag state-driven `<Modal isOpen={x}>` in a sibling.
- New API calls go through `apps/dashboard/src/libs/ApiClient.ts` — never raw `axios`/`fetch`.
- Token storage is **cookies via `js-cookie`** in `apps/dashboard/src/utils/jwt.ts` — never `localStorage`.
...
### 4. SST / infra
- Cross-component data flows through `link[]`; flag any new SSM-parameter / env-var injection that should have been a link.
- Adding a domain: extend `aliases` in `infra/domain.ts`. Never change `primaryDomain` (replaces CloudFront/cert/Route53 records).
- Handlers read `Resource.X` (the SST SDK). Flag any new `process.env.X` reads for SST-managed values, and any new `requireEnv()` calls (deprecated).
- New `LinkKey` values must be added to `linkRegistry` in `infra/api.ts` — otherwise deploy fails.
