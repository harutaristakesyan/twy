...
export const MOCK_CONFIG = {
  ...
  get ENABLE_MOCK_API(): boolean {
    if (typeof window !== 'undefined' && (window as any).__MOCK_DATA_ENABLED__ !== undefined) {
      return (window as any).__MOCK_DATA_ENABLED__
    }
...
if (typeof window !== 'undefined') {
  const initialValue = getInitialMockState()
  ;(window as any).__MOCK_DATA_ENABLED__ = initialValue
}
