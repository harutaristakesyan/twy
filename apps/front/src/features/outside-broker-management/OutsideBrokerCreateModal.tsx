...
import type { OutsideBrokerFormData } from '@/entities/outside-broker/types'
import { BrokerStatus } from '@/entities/outside-broker/types'
import { createOutsideBroker } from '@/entities/outside-broker/api'
...
const OutsideBrokerCreateModal: React.FC<OutsideBrokerCreateModalProps> = ({ visible, branches, loadingBranches, onCancel, onSuccess }) => {
  ...
  return (
    <Modal title="Add New Outside Broker" open={visible} onCancel={handleCancel} footer={null} width={600}>
      <Alert
      ...
      <Form form={form} layout="vertical" onFinish={handleSubmit} id="outsideBrokerCreateForm">
        ...
        <Form.Item name="status" label="Status" rules={[{ required: true, message: 'Please select a status' }]} initialValue={BrokerStatus.PENDING}>
