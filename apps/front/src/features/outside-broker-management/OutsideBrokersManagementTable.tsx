...
import { getOutsideBrokers, deleteOutsideBroker } from '@/entities/outside-broker/api'
...
import { hasOutsideBrokerPermission, ActionPermission } from '@/shared/utils/permissions'
...
const OutsideBrokersManagementTable: React.FC = () => {
  ...
  const [sortField, setSortField] = useState<'brokerName' | 'mcNumber' | 'status' | 'createdAt' | 'branch' | undefined>(undefined)
  ...
  // Check permissions
  const canCreate = currentUser ? hasOutsideBrokerPermission(currentUser.role, ActionPermission.CREATE) : false
  const canUpdate = currentUser ? hasOutsideBrokerPermission(currentUser.role, ActionPermission.UPDATE) : false
  const canDelete = currentUser ? hasOutsideBrokerPermission(currentUser.role, ActionPermission.DELETE) : false
  ...
  const fetchBrokers = async (overridePage?: number) => {
    ...
    try {
      ...
      if (location.pathname !== '/outside-brokers' || locationRef.current !== '/outside-brokers') {
        return
      }
  ...
  const columns: ColumnsType<OutsideBroker> = [
    ...
    },
    {
      ...
      key: 'contactName',
      render: (contactName: string | null) => (contactName ? <Text>{contactName}</Text> : <Tag color="default">No Contact</Tag>),
    },
    {
      title: 'Phone / Email',
      key: 'contact',
    ...
    },
    {
      ...
      dataIndex: 'status',
      key: 'status',
      render: (status: BrokerStatus) => <Tag color={getStatusColor(status)}>{getStatusLabel(status)}</Tag>,
      sorter: true,
