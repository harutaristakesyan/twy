...
import { LineChartOutlined, BranchesOutlined, TruckOutlined, TeamOutlined, CarOutlined } from '@ant-design/icons'
...
const Sidebar: React.FC = () => {
  ...
  const items: MenuProps['items'] = allMenuItems?.filter((item) => {
    ...
    switch (item?.key) {
      ...
      case '/outside-brokers':
        return hasMenuAccess(user.role, MenuFeature.OUTSIDE_BROKERS)
      case '/outside-carriers':
        return hasMenuAccess(user.role, MenuFeature.OUTSIDE_CARRIERS)
      default:
        return false
    }
  })
  ...
  return (
    <Sider style={siderStyle} trigger={null} collapsible collapsed={collapsed} theme="light" width={240} breakpoint="md">
      ...
      <Menu mode="inline" style={{ borderInlineEnd: 'none' }} selectedKeys={[location.pathname]} items={items} onClick={handleMenuClick} />
    </Sider>
  )
}
