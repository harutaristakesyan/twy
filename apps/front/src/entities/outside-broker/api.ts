...
export const getOutsideBrokers = async (params?: GetOutsideBrokersParams) => {
  ...
  try {
    const response = await ApiClient.get<ApiResponse<PaginatedOutsideBrokersResponse>>('/outside-brokers', queryParams)
    ...
    if (!response.data) {
      console.error('❌ Invalid response structure from getOutsideBrokers - missing data:', response)
      return { brokers: [], total: 0 }
    }
...
export const updateOutsideBroker = async (data: UpdateOutsideBrokerRequest) => {
  const response = await ApiClient.put<ApiResponse<OutsideBroker>>(`/outside-brokers/${data.id}`, data)
  return response.data
}
