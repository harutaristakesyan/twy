...
export const setAccessToken = (token: string): void =>
  cookieSet(ACCESS_TOKEN_KEY, token, { expires: expiryFromJwt(token) });

export const getRefreshToken = (): string | null => cookieGet(REFRESH_TOKEN_KEY);

export const setRefreshToken = (token: string): void =>
  cookieSet(REFRESH_TOKEN_KEY, token, { expires: expiryFromJwt(token) });

export const getIdToken = (): string | null => cookieGet(ID_TOKEN_KEY);

export const setIdToken = (token: string): void =>
  cookieSet(ID_TOKEN_KEY, token, { expires: expiryFromJwt(token) });
