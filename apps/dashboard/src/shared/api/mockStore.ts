...
import type { UpdateUserRequest, User, UserFormData, UserRole } from "@/entities/user/types";
...
class MockDataStore {
  ...
  updateUser(
    id: string,
    data: Partial<User> & Partial<UpdateUserRequest> & { branchId?: string },
  ): User | null {
    ...
    // Strip the request-only `branch` field so it doesn't end up stored on the user
    ...
    if (query) {
      filteredBranches = filteredBranches.filter(
        (branch) =>
          ...
          (branch.owner &&
            `${branch.owner.firstName} ${branch.owner.lastName}`.toLowerCase().includes(query)),
      );
    }
  ...
  createBranch(data: BranchFormData): Branch {
    ...
    if (existingBranch) {
      throw new MockHttpError('duplicate key value violates unique constraint "branch_name_key"');
    }
    ...
    if (query) {
      filteredBrokers = filteredBrokers.filter(
        (broker) =>
          ...
          broker.contactName?.toLowerCase().includes(query) ||
  ...
  createOutsideBroker(data: OutsideBrokerFormData): OutsideBroker {
    ...
    const existingBroker = this.outsideBrokers.find(
      (b) => b.mcNumber.toLowerCase() === data.mcNumber.toLowerCase(),
    );

    if (existingBroker) {
      throw new MockHttpError(
        'duplicate key value violates unique constraint "outside_broker_mc_number_key"',
      );
    }
  ...
  updateOutsideBroker(id: string, data: Partial<OutsideBrokerFormData>): OutsideBroker | null {
    ...
    if (data.mcNumber !== undefined) {
      const existingBroker = this.outsideBrokers.find(
        (b) => b.id !== id && b.mcNumber.toLowerCase() === data.mcNumber.toLowerCase(),
      );
    ...
    if (query) {
      filteredCarriers = filteredCarriers.filter(
        (carrier) =>
          ...
          carrier.equipmentType?.toLowerCase().includes(query) ||
  ...
  createOutsideCarrier(data: OutsideCarrierFormData): OutsideCarrier {
    ...
    const existingCarrier = this.outsideCarriers.find(
      (c) => c.mcDotNumber.toLowerCase() === data.mcDotNumber.toLowerCase(),
    );

    if (existingCarrier) {
      throw new MockHttpError(
        'duplicate key value violates unique constraint "outside_carrier_mc_dot_number_key"',
      );
    }
  ...
  updateOutsideCarrier(id: string, data: Partial<OutsideCarrierFormData>): OutsideCarrier | null {
    ...
    if (data.mcDotNumber !== undefined) {
      const existingCarrier = this.outsideCarriers.find(
        (c) => c.id !== id && c.mcDotNumber.toLowerCase() === data.mcDotNumber.toLowerCase(),
      );
    ...
    const insuranceExpiry =
      data.insuranceExpiry !== undefined ? data.insuranceExpiry : carrier.insuranceExpiry;
    if (insuranceExpiry) {
    ...
    this.outsideCarriers[index] = {
      ...
      equipmentType:
        data.equipmentType !== undefined ? data.equipmentType || null : carrier.equipmentType,
      insuranceStatus,
