...
export const getTeamMembers = async (
  ...
  if (params?.limit !== undefined) queryParams.limit = params.limit;
  if (params?.query) queryParams.query = params.query;
  const response = await ApiClient.get<ApiResponse<TeamMemberListResponse>>(
...
export const addTeamMember = async (teamId: string, userId: string): Promise<void> => {
  await ApiClient.post<ApiResponse<void>>(`/teams/${teamId}/members`, { userId });
};

export const removeTeamMember = async (teamId: string, userId: string): Promise<void> => {
  await ApiClient.delete<ApiResponse<void>>(`/teams/${teamId}/members/${userId}`);
};

export const getUnassignedUsers = async (params?: {
  page?: number;
  limit?: number;
  query?: string;
}): Promise<TeamMemberListResponse> => {
