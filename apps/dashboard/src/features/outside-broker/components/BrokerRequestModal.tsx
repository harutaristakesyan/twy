...
const BrokerRequestModal = () => {
  ...
  const { data: rec, isLoading } = useApiQuery(queryKeys.brokerRequests.detail(requestId), () =>
  ...
  const refreshLists = () =>
    invalidate(
      queryKeys.brokerRequests.all,
      queryKeys.brokerRequests.detail(requestId),
      queryKeys.outsideBrokers.all,
    );
      ...
      >
        <Modal.Container>
          <Modal.Dialog>
            <Modal.CloseTrigger />
            <Modal.Header>
              <Modal.Heading>Broker Request</Modal.Heading>
            </Modal.Header>
              ...
              {isLoading ? (
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex items-start justify-between rounded-xl border border-default-200 bg-default-50 px-4 py-3">
                    <div>
                      <p className="text-base font-semibold text-default-900">{rec.brokerName}</p>
                      <p className="mt-0.5 text-sm text-default-500">MC {rec.mcNumber}</p>
                    </div>
                    <Chip
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="grid grid-cols-2 gap-x-6 gap-y-4 rounded-xl border border-default-100 p-4">
                      <InfoField label="Contact Name" value={rec.contactName} />
                      <InfoField label="Phone" value={rec.phone} />
                      <InfoField label="Email" value={rec.email} span={2} />
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="grid grid-cols-2 gap-x-6 gap-y-4 rounded-xl border border-default-100 p-4">
                      ...
                      />
                      {rec.address && <InfoField label="Address" value={rec.address} span={2} />}
