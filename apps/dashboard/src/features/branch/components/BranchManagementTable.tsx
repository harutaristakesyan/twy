...
import type { BranchColumnDef } from "./useBranchColumns";

type BranchManagementTableProps = {
  items: Branch[];
  columns: BranchColumnDef[];
  renderCell: (record: Branch, colId: string) => ReactNode;
  total: number;
  page: number;
  pageSize: number;
  isLoading: boolean;
...
const BranchManagementTable: React.FC<BranchManagementTableProps> = ({
  items,
  columns,
  renderCell,
  total,
  ...
  return (
    <Table>
      <Table.ScrollContainer>
        <Table.Content aria-label="Branches" className="min-w-full">
