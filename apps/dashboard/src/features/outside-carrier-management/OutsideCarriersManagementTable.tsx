...
import { deleteOutsideCarrier, getOutsideCarriers } from "@/entities/outside-carrier/api";
...
import { ActionPermission, hasOutsideCarrierPermission } from "@/shared/utils/permissions";
...
const OutsideCarriersManagementTable: React.FC = () => {
  ...
  const [sortField, setSortField] = useState<
    "carrierName" | "mcDotNumber" | "status" | "insuranceStatus" | "createdAt" | undefined
  >(undefined);
  ...
  const canCreate = currentUser
    ? hasOutsideCarrierPermission(currentUser.role, ActionPermission.CREATE)
    : false;
  const canUpdate = currentUser
    ? hasOutsideCarrierPermission(currentUser.role, ActionPermission.UPDATE)
    : false;
  const canDelete = currentUser
    ? hasOutsideCarrierPermission(currentUser.role, ActionPermission.DELETE)
    : false;
  ...
  const fetchCarriers = useCallback(
    async (overridePage?: number) => {
      ...
      try {
        ...
        if (
          location.pathname !== "/outside-carriers" ||
          locationRef.current !== "/outside-carriers"
        ) {
          return;
        }
  ...
  }, [fetchCarriers, location.pathname]);
  ...
  const handleTableChange = (
    ...
    sorter: SorterResult<OutsideCarrier> | SorterResult<OutsideCarrier>[],
  ) => {
        ...
        <div style={{ marginBottom: 16 }}>
          <Row justify="space-between" align="middle">
            <Col>
              ...
              <Text type="secondary">
                Manage outside carriers and their status. System uses this data to control load
                assignments.
              </Text>
            </Col>
            <Col>
