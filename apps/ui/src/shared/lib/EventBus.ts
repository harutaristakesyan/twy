import type { ThemeMode } from "ahooks/lib/useTheme";
...
export interface EventPayloads {
  ...
  [EventType.UserStocksChanged]: undefined;
}
...
export const onEvent = bus.on.bind(bus) as <K extends EventKey>(
  event: K,
  callback: EventCallback<K>,
) => void;

export const offEvent = bus.off.bind(bus) as <K extends EventKey>(
  event: K,
  callback: EventCallback<K>,
) => void;
...
export function useEvent<K extends EventKey>(
  events: K[],
  callback: (event: K, payload: EventPayloads[K]) => void,
): void;
